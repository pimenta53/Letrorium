...
int vp(Elem **Tab,int *DIM) {
	...
	for (i=0;i<((*DIM)-1);i++) {
		for (j=0;j<((*DIM)-1);j++) {
			if ((*Tab)[i][j].cor=='p') {
				if (i!=((*DIM)-1)) {
					if ((*Tab)[i+1][j].cor=='p') return mensagem_de_erro(E_WRONG_SOLUTION);
				}
				if (j!=((*DIM)-1)) {
					if ((*Tab)[i][j+1].cor=='p') return mensagem_de_erro(E_WRONG_SOLUTION);
				}
			}
		}
	}
...
int st(Elem **Tab, int *DIM) {
	
	if (vb(Tab,DIM)) return 1;/*sair para não mandar varias mensagens de erro caso não passe nos testes todos*/
	if (vp(Tab,DIM)) return 1;
	if (vl(Tab,DIM)) return 1;
