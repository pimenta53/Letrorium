...
int executaJogada(char *args,char *cor,int *DIM,Elem **Tab,int *flagAP,int *flagAB,Gravados *gr) {
  ...
  if(nargs != 2) {
	...
	if(!sair) control(Tab,DIM,flagAB,flagAP,gr);
...
int executa_comando(char *linha,int *DIM,Elem **Tab,int *flagAP,int *flagAB,Gravados *gr) {
  char cmd[1025],args[1025];
	...
	if((strcmp(cmd, "b") == 0 || strcmp(cmd, "p") == 0 || strcmp(cmd, "i") == 0))
		return executaJogada(args,cmd,DIM,Tab,flagAP,flagAB,gr);
	if(strcmp(cmd, "cr") == 0 )
		return executaCriaJogo(args,DIM,Tab,gr,flagAP,flagAB);	
	if(strcmp(cmd,"gr")==0 )
	...
	if(strcmp(cmd,"vl")==0 && nargs==1)
		return vl(Tab,DIM);
	if(strcmp(cmd,"st")==0 && nargs==1)
		return st(Tab,DIM);
	if(strcmp(cmd,"anc")==0 && nargs==1)
		return anc(Tab,gr);
	if(strcmp(cmd,"anm")==0 && nargs==1)
		return anm(Tab,gr);
	if(strcmp(cmd,"it")==0 && nargs==1)
		return it(gr);
	if(strcmp(cmd,"rb")==0 && nargs==1)
		return rb(Tab,gr);
	if(strcmp(cmd,"q")==0){
		free_jogadasGR(gr);
		freeTab(DIM,Tab);
