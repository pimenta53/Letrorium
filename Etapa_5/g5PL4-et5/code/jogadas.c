 ...
 * @param cores contador de cores mudadas 
 * @returns Retorna um inteiro que verifica se a jogada foi ou não bem sucedida.
 */
int jogada(char cor,int x,int y,Elem **Tab,Gravados *gr,int erro,int *cores){
	
	
	if((*Tab)[y-1][x-1].cor==cor){
		 if(cores) cores++;
		 return -1;
	}
	if(cor=='i'){
	...
	if(cor=='b'){
		...
		else if(erro)return mensagem_de_erro(E_WRONG_SOLUTION);
			else return 1;
	}
	if(cor=='p'){
...
/**
 * Função responsável por verificar se existe um triplo a partir das cordenadas i e j na vertical.
 ...
 * @param gr apontador para a estrutura de dados onde se guardas as jogadas e movimentos.
 * @returns inteiro que irá retornar se houve alguma alteração no tabuleiro ou não
 */
int E_trp_col(Elem **Tab,int *DIM,int i,int j,Gravados *gr){
	...
	if((j>0)&&j+1<(*DIM)){
		...
		if(c1!=c5 && c1==c2 && c2==c3 && c3!=c4){
			mudou1=jogada('p',j,i+1,Tab,gr,1,NULL);
			mudou2=jogada('b',j+1,i+1,Tab,gr,1,NULL);
			mudou3=jogada('p',j+2,i+1,Tab,gr,1,NULL);
		}
	}
...
/**
 * Função responsável por verficar se existe um triplo a partir das cordenadas i e j na horinzontal.
 * Se existir atribui as cores correspondentes.
 * @param Tab Tabuleiro do jogo
 * @param DIM dimensão do tabuleiro
...
int E_trp_row(Elem **Tab,int *DIM,int i,int j,Gravados *gr){
	...
	if((i>0)&&i+1<(*DIM)){
		...
		if(c1!=c5 && c1==c2 && c2==c3 && c3!=c4){
			mudou1=jogada('p',j+1,i,Tab,gr,1,NULL);
			mudou2=jogada('b',j+1,i+1,Tab,gr,1,NULL);
			mudou3=jogada('p',j+1,i+2,Tab,gr,1,NULL);
		}
	}
	if(mudou1==-1 && mudou1==mudou2 && mudou2==mudou3) return 1;
	else return 0;
...
int trp(Elem **Tab,int *DIM,int *flagAP,int *flagAB,Gravados *gr){
	...
	if(!sair1 && !sair2) control(Tab,DIM,flagAB,flagAP,gr,1);
		...
		for(j=0;j<(*DIM);j++){	
			if((i-1>=0)&&(i+1<(*DIM))){
				if((*Tab)[i-1][j].letra==(*Tab)[i+1][j].letra && (*Tab)[i][j].letra!=(*Tab)[i+1][j].letra){
					sair1=jogada('b',j+1,i+1,Tab,gr,1,NULL);
					if(sair1>0) return 1;
				}
			}
			if((j-1>=0)&&j+1<(*DIM)){
				if((*Tab)[i][j-1].letra==(*Tab)[i][j+1].letra && (*Tab)[i][j].letra!=(*Tab)[i][j+1].letra){
					sair2=jogada('b',j+1,i+1,Tab,gr,1,NULL);
					if(sair2>0) return 1;
				}
			}
		}
	...
	for(i=0;i<(*DIM);i++){
		for(j=0;j<(*DIM);j++){
			...
			if((c4!=c2 && c2!=c5) && E_Par(Tab,i,c2,DIM,"linha")){
				sair1=jogada('p',j+1,i+1,Tab,gr,1,NULL);
				if(sair1>0) return 1;
			}
			if((c1!=c2 && c2!=c3) && E_Par(Tab,j,c2,DIM,"coluna")){
				sair2=jogada('p',j+1,i+1,Tab,gr,1,NULL);
				if(sair2>0) return 1;
			}
		}
	}
	...
	for(i=0;i<(*DIM)-1;i++){
		for(j=0;j<(*DIM)-1;j++){
			if(E_Par_des(Tab,(*Tab)[i][j].letra,(*Tab)[i+1][j+1].letra,DIM,i,j,i+1,j+1)){ /*i e j para vereficar se são diferentes das posiçoes passadas*/
				mudou1=jogada('b',j+2,i+1,Tab,gr,1,NULL);
				if(mudou1>0) return 1;
				mudou2=jogada('b',j+1,i+2,Tab,gr,1,NULL);
			...
			if(E_Par_des(Tab,(*Tab)[i+1][j].letra,(*Tab)[i][j+1].letra,DIM,i+1,j,i,j+1)){ /*i e j para vereficar se são diferentes das posiçoes passadas*/
				mudou1=jogada('b',j+1,i+1,Tab,gr,1,NULL);
				if(mudou1>0) return 1;
				mudou2=jogada('b',j+2,i+2,Tab,gr,1,NULL);
				if(mudou1>0) return 1;
